...
# Sincronizar VM02
sshpass -p 'Password2012' ssh -p 5001 soporte@10.0.0.7 '
...
## Planificación y Diseño mediante Subagentes Especializados
*   **Uso Obligatorio de `elitepass-plan-generator`**: Para cualquier plan de desarrollo o mejora técnica (cambios de base de datos, seguridad, integraciones o performance), debes leer la skill [elitepass-plan-generator](file:///home/soporte/.agents/skills/elitepass-plan-generator/SKILL.md) y utilizarla para diseñar el plan de forma modular utilizando los subagentes especializados coordinados por el Project Manager, antes de redactar código o ejecutar migraciones en el workspace.

## Memoria y Sincronización Post-Ejecución (Handoff)
*   **Guardado Obligatorio de Memoria**: Inmediatamente después de terminar las fases de un plan o modificaciones en el código, debes actualizar o crear un archivo de memoria (`MEMORY.md` u `openspec/changes/<task-name>/tasks.md` actualizados) detallando exhaustivamente qué se ha completado, qué está pendiente, las decisiones tomadas y los pasos a seguir. Esto garantiza que la próxima sesión (sea con Claude o Gemini) pueda continuar el trabajo sin pérdida de contexto.
